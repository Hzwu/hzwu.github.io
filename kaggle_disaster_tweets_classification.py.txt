...
trn_file = './tasks/Kaggle-Natural-Language-Processing-with-Disaster-Tweets/train.csv'
tst_file = './tasks/Kaggle-Natural-Language-Processing-with-Disaster-Tweets/test.csv'

df_trn = pd.read_csv(trn_file)
df_tst = pd.read_csv(tst_file)
df_trn['merged_text'] = df_trn['keyword'].astype(str) + ' [A] ' + df_trn['location'].astype(str) + ' [B] ' + df_trn[
    'text'].astype(str)
df_tst['merged_text'] = df_tst['keyword'].astype(str) + ' [A] ' + df_tst['location'].astype(str) + ' [B] ' + df_tst[
    'text'].astype(str)

df_trn.drop(columns=['id', 'keyword', 'location', 'text'], inplace=True)
df_tst.drop(columns=['id', 'keyword', 'location', 'text'], inplace=True)
...
dataset_trn = DisasterTweetsDataset(df_trn['merged_text'].values, df_trn['target'].values)
dataset_tst = DisasterTweetsDataset(df_tst['merged_text'].values)
...
for nb_epoch in range(32):
    acc = train(loader_trn, loader_val, nb_epoch)
    if acc > 0.8:
        break

pred_list = test(loader_tst)
df_tst['target'] = [idx for idx in pred_list]
df_tst.drop(columns='merged_text', inplace=True)
sub_file = './tasks/Kaggle-Natural-Language-Processing-with-Disaster-Tweets/sample_submission_2.csv'
